...
def build_libs():
    for name, cfg in LIB_TARGETS.items():
        ...
        if arch != 'riscv':
            ...
            cmd = f"gcc-14 {cfg['flags']} -fPIC -shared -march=rv64imafdcv_zvfh_zfh -o {cfg['out']} {cfg['src']}"
        cmd += " -lm"
...
ffi.cdef("void " + OPSNAME + "(float* input, float* output, int m,int n);")
...
class ops_C:
    def __init__(self, lib, ffi, tensor): #[batch,exhidden_dim,hidden_dim]
        ...
        self.ffi = ffi
        self.tensor = tensor
        self.m, self.n = tensor.shape
        self.out = np.empty((self.m, self.n), dtype=np.float16)

    def __call__(self):
        inp_ptr = self.ffi.cast("void*", self.ffi.from_buffer(self.tensor))
        out_ptr = self.ffi.cast("float*", self.ffi.from_buffer(self.out))
        self.lib.__getattr__(OPSNAME)(inp_ptr, out_ptr, self.m, self.n)
        return self.out

# --------------------------
# 3. Python 实现
# --------------------------
def ops_python(tensor): 

    
    return ops_numpy(tensor)  
...
def run_test(input_tensor, libs, repeat=50, diff_threshold=1e-5):
    ...
    for name, lib in libs.items():
        ...
        diff_radion = diff / (np.max(np.abs(y_py)) + 1e-6)
        print(f"[{name}] time={t*1e3:.3f} ms  diff={diff:.2e} diff_radion={diff_radion:.2e}")
        if diff_radion > diff_threshold or (math.isnan(diff) ):
