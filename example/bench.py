...
def run_test(x, libs, repeat=50, diff_threshold=1e-5):
    ...
    # Python
    t_py, y_py = benchmark(lambda b: np.array(ops_python(b), dtype=np.float32), x.tolist(), repeat)
    print(f"[Python-native] time={t_py*1e3:.3f} ms")

    # NumPy
    t_np, y_np = benchmark(lambda arr: ops_numpy(arr), x, repeat)
    diff_np = np.max(np.abs(y_py - y_np))
    print(f"[NumPy] time={t_np*1e3:.3f} ms  diff={diff_np:.2e}")

    # C实现
    for name, lib in libs.items():
        func = ops_C(lib, ffi, x.shape[0], x.shape[1])
