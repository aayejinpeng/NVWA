...
void fuse_shift_scale_A_left_scale_1_resadd_relu(int32_t * input, int8_t * output, uint64_t stride_input, uint64_t stride_output,uint64_t shift_scale, uint64_t dim_I,int8_t *residual)
{
    ...
    for(int i = 0;i<dim_I;i++)
    {
        ...
        while (j < 64) {
            ...
            // 2. right shift
            vint8m1_t vec_shifted = __riscv_vsll_vx_i8m1(__riscv_vnclip_wx_i8m1(__riscv_vnclip_wx_i16m2(vec_input,shift_scale,vl), 0, vl),1,vl);
            // 3. load residual
...
void fuse_shift_scale_A_left_scale_1_resadd_relu(int32_t * input, int8_t * output, uint64_t stride_input, uint64_t stride_output,uint64_t shift_scale, uint64_t dim_I,int8_t *residual)
{
    ...
    for(int i = 0;i<dim_I;i++)
    {
        ...
        for(int j = 0;j<64;j++)
        {
            int8_t val = (int8_t)((row_input[j] + (1 << (shift_scale - 1))) >> shift_scale);
            val = val << 1;
            val = val + row_residual[j];
            if(val < 0) val = 0;
            if(val > 127) val = 127;
