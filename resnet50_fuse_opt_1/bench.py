...
OPSNAME="fuse_shift_scale_resadd_relu"
...
def ops_python(input,stride_input,stride_output,shift_scale,dim_I,residual): 
    ...
    for i in range(dim_I):
        for j in range(64):
            val = ((input[i, j] + (1 << (shift_scale - 1))) >> shift_scale) + residual[i, j]
...
if __name__ == "__main__":
    ...
    run_test(input_tensor, stride_input, stride_output, shift_scale, dim_I, residual, libs, repeat=50, diff_threshold=1e-5)
